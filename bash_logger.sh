...
#XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    ...
    if [ $# -eq 2 ]; then
		...
		([ "$logLevel" = "STDOUT" ] && echo "$1 [`date +'%d-%b-%y %T'`] $2" | tee -a $logFileList) || echo "$1 [`date +'%d-%b-%y %T'`] $2" | tee -a $logFileList >> /dev/null
    else
        ([ "$logLevel" = "STDOUT" ] && echo "log [`date +'%d-%b-%y %T'`] $@" | tee -a $debugLogFile) || ([ "$logLevel" = "DEBUG" ] && echo "DEBUG [`date +'%d-%b-%y %T'`] $@" | tee -a $debugLogFile >> /dev/null)
 	fi
}
